...
/* panel for the snapshot widget lets you create snapshots and overwrite
 * existing ones */
let snapPanel = panel.Panel({
  ...
  onMessage: function(data) {
    if (data.snap) {
      /* a new snapshot needs to be created */
      ...
      /* an existing snapshot needs to be overwritten */
      ...
      let visibleTabs = gBrowser.visibleTabs;
      let moveTabs = [];
      visibleTabs.filter(function(tab) {
        return (!tab.pinned);
...
/* return a list of all bookmarks folders */
...
/* given a bookmark folder id, return bookmarks in it */
...
/* gallery panel from where the user can open bookmark folders into a panorama */
let galleryPanel = panel.Panel({
  ...
  onShow: function() {
    /* send the folders to the html file so it can populate the panel */
    this.postMessage({
      "folders" : getBookmarkFolders(),
    });
  },
  onMessage: function(data) {
    /* opens up a bookmark folder into a new panorama group */
    let me = this;
    let urls = getGroupItems(data.id);
    let currentWindow = Svcs.Services.wm.getMostRecentWindow("navigator:browser");

    /* creates the panorama group and populates it, panorama may or may not
     * required init before this is run */
    function initTabs() {
    ...
    /* initialize panorama if not ready */
