...
function setLRU(folderId) {
  let order = new Array(ss.storage.lru);
  reportError("BEFORE: "  + order);
  order = order.filter(function (a) {
    return (a != folderId);
  }).push(folderId);
  reportError("AFTER" + order);
  ss.storage.lru = order;
}

function getLRUIndex(folderId) {
  let order = new Array(ss.storage.lru);
  return order.indexOf(folderId);
}
...
function getBookmarkFolders() {
  return utils.spinQuery(Places.PlacesUtils.history.DBConnection, {
    ...
    return getLRUIndex(b) - getLRUIndex(a);
  });
}
